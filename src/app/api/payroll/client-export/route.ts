...
import {
  buildPayrollCoverPdf,
  buildPayrollDetailPdf,
  buildPayrollSummaryPdf,
} from "../../../../lib/exports/simplePdf";
...
async function requireAdmin(req: Request) {
  ...
  if (profErr) {
    return { ok: false as const, status: 400, error: profErr.message };
  }
  if (!prof?.org_id) {
    return { ok: false as const, status: 403, error: "No org" };
  }
  if (prof.role !== "admin") {
...
export async function GET(req: Request) {
  try {
    ...
    if (!period_start || !period_end || !project_id) {
      return NextResponse.json(
        { ok: false, error: "Missing period_start/period_end/project_id" },
        { status: 400 }
      );
    }
    ...
    if (ppErr) {
      return NextResponse.json(
        { ok: false, error: ppErr.message },
        { status: 400 }
      );
    }
    if (!pp?.locked) {
      return NextResponse.json(
        { ok: false, error: "Pay period is not locked" },
        { status: 409 }
      );
    }
    ...
    if (!run?.id) {
      return NextResponse.json(
        { ok: false, error: "No payroll run snapshot" },
        { status: 409 }
      );
    }
    ...
    const { data: ent, error: entErr } = await supa
      ...
      .select(
        "time_entry_id,entry_date,contractor_id,contractor_name_snapshot,project_name_snapshot,project_id,hours,hourly_rate_snapshot,amount"
      )
    ...
    const by = new Map<
      string,
      { contractor_id: string; contractor: string; hours: number; pay: number }
    >();
    ...
    for (const r of ent ?? []) {
      ...
      const pay = Number(
        (r as any).amount ??
          hours * Number((r as any).hourly_rate_snapshot ?? 0)
      );

      const cur = by.get(cid) || {
        contractor_id: cid,
        contractor: name,
        hours: 0,
        pay: 0,
    ...
    const totalHours = summaryRows.reduce(
      (a, s) => a + Number(s.total_hours || 0),
      0
    );
    const totalPay = summaryRows.reduce(
      (a, s) => a + Number(s.total_pay || 0),
      0
    );

    const cover = buildPayrollCoverPdf(pdfMeta as any, {
      totalHours,
      totalPay,
      currency: (run as any)?.currency || "USD",
    });
    ...
    const zip = buildZip([
      ...
      {
        name: `payroll_${safeProject}_${stamp}_summary.csv`,
        data: Buffer.from(summaryCsv, "utf8"),
      },
      {
        name: `payroll_${safeProject}_${stamp}_detail.csv`,
        data: Buffer.from(detailCsv, "utf8"),
      },
      {
        name: `payroll_${safeProject}_${stamp}_summary.pdf`,
        data: pdf,
      },
      {
        name: `payroll_${safeProject}_${stamp}_detail.pdf`,
        data: pdfDetail,
      },
      {
        name: `manifest_${safeProject}_${stamp}.json`,
    ...
    try {
      ...
      if (!peErr) {
        projectExportId = (pe as any)?.id ?? null;
      }
    } catch {
