...
import { buildOpsNotifications } from "../../lib/opsNotifications";
...
function AnalyticsPageContent() {
  ...
  const summary = useMemo(() => {
    ...
    return { totalHours, approvedHours, submittedHours, totalCost, people, projects, hoursDelta, costDelta, byProject, byPerson, contractorConcentration, budgetedProjects: budgetedProjects.length, overBudgetProjects: overBudgetProjects.length, nearBudgetProjects: nearBudgetProjects.length, totalBudgetAmount, pendingForecast, projectedPayroll, staleApprovals };
  ...
  const notifications = buildOpsNotifications({
    contractors: summary.byPerson.map((item) => ({ id: item.id, full_name: item.name })),
    rows,
    budgets: Object.entries(projectBudgets).map(([id, budget]) => ({ id, ...budget })),
            ...
            <>
              ...
              <div className="setuSignalGrid" style={{ marginBottom: 18 }}>
                <div className="setuSignalCard">
                  <div className="setuSignalLabel">Forecast movement</div>
                  <strong>{formatMoney(summary.pendingForecast)}</strong>
                  <span>Submitted work still outside the locked payroll baseline for this range.</span>
                </div>
                <div className="setuSignalCard">
              ...
              <div className="card cardPad" style={{ marginBottom: 18 }}>
                <div className="setuCardHeaderRow" style={{ marginBottom: 12 }}>
                  <div><div className="setuSectionTitle" style={{ fontSize: 20 }}>Management signals</div><div className="setuSectionHint">The same alert model now powers Dashboard, Projects, and the notification center groundwork.</div></div>
                  <Button variant="secondary" onClick={() => (window.location.href = "/admin/notifications")}>Notifications</Button>
                </div>
                  ...
                  {notifications.slice(0, 4).map((item) => (
                    <button key={item.id} className={`setuNotificationItem severity-${item.severity}`} onClick={() => (window.location.href = item.href)}>
                      <div style={{ display: "grid", gap: 6, flex: 1 }}>
                        <div className="setuNotificationHead"><span className="setuNotificationTitle">{item.title}</span><span className={`setuNotificationBadge severity-${item.severity}`}>{item.metric}</span></div>
                        <div className="setuProjectMeta">{item.body}</div>
                      </div>
                    </button>
                  ))}
                  {!notifications.length ? <div className="muted">No additional management signals in this range.</div> : null}
                </div>
              </div>
